...
#include <immintrin.h>
#include "trace2.h"
extern void ct_mark(const char *);
...
#define V_MINMAX(a,b) do { ct_vmm(a,b); } while (0)
...
static void rev_pass(int32 *x, long long n, int p)
{
  ...
  for (long long z = 0; z < n; z += 16) {
    int32x8 v[2];
    v[0] = ct_xor(V_load(&x[z]), m);
    v[1] = ct_xor(V_load(&x[z+8]), m);
...
static void sort_2power(int32 *x, long long n, int flagdown)
{
  ...
  if (n == 8) {
    static const unsigned char p8[19][2] =
      { {1,0},{3,2},{2,0},{3,1},{2,1},
        ...
        {5,1},{7,3},{5,3},
        {2,1},{4,3},{6,5} };
    for (int t = 0; t < 19; t++) s_minmax(&x[p8[t][1]], &x[p8[t][0]]);
    return;
  }
  ...
  if (n == 16) {
    ...
    v[0] = ct_xor(V_load(&x[0]),m);
    v[1] = ct_xor(V_load(&x[8]),m);
    ...
    if (flagdown) v[1] = ct_xor(v[1],mask);
    else          v[0] = ct_xor(v[0],mask);
  ...
  if (n == 32) {
    ...
    if (flagdown) {
      x0 = ct_xor(x0,mask); x1 = ct_xor(x1,mask);
      x2 = ct_xor(x2,mask); x3 = ct_xor(x3,mask);
    }
    V_MINMAX(x0,x2);
    V_MINMAX(x1,x3);
...
ct_mark("doubling merges");
    if (n >= 128) {
      for (j = 0; j < n; j += 32) {
        V_store(&x[j],    ct_xor(V_load(&x[j]),mask));
        V_store(&x[j+16], ct_xor(V_load(&x[j+16]),mask));
      }
        ...
        for (j = 0; j < q; j += p + p) {
          for (k = j; k < j + p + p; k += p) {
            for (i = k; i < k + p; i += 8) {
              ...
              vnet(v,N_mrg8r);
              if (flip) for (int t = 0; t < 8; t++) v[t] = ct_xor(v[t],mask);
...
ct_mark("transpose + sort");
    for (i = 0; i < n; i += 64) {
      int32x8 v[8];
      for (int t = 0; t < 8; t++) v[t] = V_load(&x[i + 8*t]);
      tr_lo(v);
      if (flagdown) {
        v[2] = ct_xor(v[2],mask); v[3] = ct_xor(v[3],mask);
        v[6] = ct_xor(v[6],mask); v[7] = ct_xor(v[7],mask);
      } else {
        v[0] = ct_xor(v[0],mask); v[1] = ct_xor(v[1],mask);
        v[4] = ct_xor(v[4],mask); v[5] = ct_xor(v[5],mask);
      }
      tr_hi(v);
      vnet(v,N_mrg8r);
      transpose8(v);
    ...
    ct_mark("final ladder");
    ladder(x,n);

    ct_mark("sort + transpose out");
    {
      long long q = n >> 3;
      ...
      for (i = 0; i < q; i += 8) {
        ...
        vnet(v,N_mrg8r);
        transpose8p(v);
        if (flagdown) for (int t = 0; t < 8; t++) v[t] = ct_xor(v[t],mask);
...
void int32_sort_dbg(int32 *x, long long n)
{
  if (n <= 8) {
