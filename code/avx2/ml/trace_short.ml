let trace : Buffer.t = Buffer.create (1 lsl 20)
let ncmp = ref 0
let emit ca cb =
...
(* the same, on [cnt] consecutive scalars starting at [b] *)
let snet mem b pairs = List.iter (fun (i, j) -> sminmax mem (b + i) (b + j)) pairs
...
let mrg2  = [ 0,1 ]
let tail8 = [ 0,4; 1,5; 2,6; 3,7;  0,2; 1,3;  0,1; 2,3;  4,6; 5,7;  4,5; 6,7 ]
...
(* sort.c open-codes an 8x8 in-register transpose as a cascade of             *)
(* unpacklo/hi_epi32, then _epi64, then permute2x128.  Composed, the cascade   *)
(* is just the transpose; [tr_lo] and [tr_hi] are its two halves, needed apart *)
(* because one use interleaves a sign-flip between them.  Each is a fixed      *)
(* index permutation, [sw] swapping the middle two rows of each group of four. *)
let sw = [| 0; 2; 1; 3 |]

let tr_lo (v : v array) : v array =
  Array.init 8 (fun r -> Array.init 8 (fun c ->
    v.(4 * (r / 4) + c mod 4).(4 * (c / 4) + sw.(r mod 4))))

let tr_hi (v : v array) : v array =
  Array.init 8 (fun r -> Array.init 8 (fun c ->
    v.(4 * (c / 4) + sw.(r mod 4)).(4 * (r / 4) + c mod 4)))

let transpose8 (v : v array) : v array =
  Array.init 8 (fun r -> Array.init 8 (fun c -> v.(c).(r)))

(* The variant sort.c uses in the final block: the input arrives strided and   *)
(* the output lands scrambled, so the transpose is composed with a fixed       *)
(* permutation on each side.                                                   *)
let tr'r = [| 0; 2; 1; 3; 4; 6; 5; 7 |]
let tr'c = [| 0; 4; 2; 6; 1; 5; 3; 7 |]

let transpose8' (v : v array) : v array =
  Array.init 8 (fun r -> Array.init 8 (fun c -> v.(tr'c.(c)).(tr'r.(r))))

(* ------------------------------------------------------------------------- *)
(* The reversing passes (sort.c lines 514-638).  Each flips a size-2p pattern  *)
(* inside every 16 lanes, then compares across it.  Bringing the wires to be   *)
(* compared into matching lanes takes p = 4: nothing, p = 2: the 128-bit       *)
(* permute, p = 1: that and the 64-bit unpack.  Once down, a compare-exchange  *)
(* is done at every level on the way back up.                                  *)
(* ------------------------------------------------------------------------- *)
let rev_pass mem off n p =
  let m = match p with
    | 4 -> set_epi32 0 0 0 0 (-1) (-1) (-1) (-1)
    | 2 -> set_epi32 0 0 (-1) (-1) (-1) (-1) 0 0
    | _ -> set_epi32 0 (-1) (-1) 0 0 (-1) (-1) 0 in
  let levels = match p with 4 -> [] | 2 -> [`Perm] | _ -> [`Perm; `Unpack64] in
  let z = ref off in
  while !z <> off + n do
    let a = ref (vxor (load mem !z) m)
    and b = ref (vxor (load mem (!z + 8)) m) in
    let step = function
      | `Perm -> let x = permute2x128 !a !b 0x20 and y = permute2x128 !a !b 0x31 in
                 a := x; b := y
      | `Unpack64 -> let x = unpacklo_epi64 !a !b and y = unpackhi_epi64 !a !b in
                     a := x; b := y in
    List.iter step levels;
    List.iter (fun l -> let (x, y) = minmax !a !b in a := x; b := y; step l)
      (List.rev levels);
    store mem !z !a; store mem (!z + 8) !b;
    z := !z + 16
  done
...
and int32_sort (mem : int array) off n : unit =
  if n <= 8 then begin
    ...
    for len = n downto 2 do
      for i = 0 to len - 2 do mm i (i + 1) done
    done
  end
    ...
    if !q <= 128 then begin
      ...
      let j = ref 0 in
      let w = ref (!q asr 2) in
      while !w >= 2 do
        let first = if !w = 8 then mrg8 else if !w = 4 then mrg4 else mrg2 in
        while !j + 8 * !w <= n do bmerge mem off !j !w first; j := !j + 8 * !w done;
        minmax_vector mem (off + !j) (off + !j + 4 * !w) (n - 4 * !w - !j);
        w := !w asr 1
      done;
      if !j + 8 <= n then (snet mem (off + !j) tail8; j := !j + 8);
      minmax_vector mem (off + !j) (off + !j + 4) (n - 4 - !j);
      if !j + 4 <= n then (snet mem (off + !j) mrg4; j := !j + 4);
