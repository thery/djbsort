...
(******************************************************************************)
(*                                                                            *)
(*  sort_transpose.v -- sort_transpose.ml computes the network pbsort         *)
(*                                                                            *)
(*      tsort_avx2_pbsort :                                                   *)
(*        tsort tmerge_avx2 false t = nfun (pbsort false k) t                 *)
(*                                                                            *)
(*  whence avx2_sort_sorted and avx2_sort_perm, and their padded forms.       *)
(*                                                                            *)
(*  The transpose changes nothing about the network: the same line pairs are  *)
(*  compared, only the execution differs -- flip the descending lanes,        *)
(*  transpose the m x m block so a within-lane comparator becomes a           *)
(*  cross-vector one, take a uniform min/max, transpose back, unflip.  So     *)
(*  there is no second sorting theorem, only the identification of what that  *)
(*  execution computes: the PERIODIC net pbsort of sort_generic.v, sort_      *)
(*  transpose.ml's direction rule being `i land k`, the block-parity rule,    *)
(*  not the reflected bfsort.                                                 *)
(*                                                                            *)
(*  The algebra this rests on -- transpose, sign flip, rows and columns,      *)
(*  tiling and reshape -- is in common/nalgebra.v.  What is here is the part  *)
(*  that is about the program:                                                *)
(*                                                                            *)
(*    sqmerge, sqblock_reify_luni  the single square block and its reified    *)
(*                                 transposed / flipped execution             *)
(*    sqpow, sqcast                retyping that block for tiling             *)
(*    ph_asc, tmerge_phase         one ascending / descending phase           *)
(*    tmerge_avx2, tsort           the merge and the sort themselves          *)
(*                                                                            *)
(******************************************************************************)
