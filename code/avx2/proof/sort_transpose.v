...
(*       computes the same as a plain periodic connector schedule.  This is   *)
(*       what cfun_conj / nttr / nrows / ncols above are for: a whole-vector  *)
(*       or cross-vector stage is nrows-shaped, a sub-lane stage is ncols-    *)
(*       shaped with the sign-flip mask folded in by cfun_conj.               *)
(*  (R2) Plain iterative = recursive pbsort.  The plain periodic k/j schedule *)
(*       (shared with sort_generic.ml) folds to nfun (pbsort k) -- the        *)
(*       iterative-vs-recursive bitonic bridge, the real content.             *)
(*                                                                            *)
